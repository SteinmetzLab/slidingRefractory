...
def plotHillComp(pcReg,pcHill, savefile, recDurPlot = 2, rpPlot = 2,frPlot = 10,highCont=False):
    ...
    cHill = cc.linear_tritanopic_krw_5_95_c46
    cHill = cHill[::-1]  # if using linear_blue37 or protanopic, flip the order
    colorsHill = [cHill[x] for x in np.round(np.linspace(0.2, 0.75, len(frs)) * 255).astype(int)]
