...
Replaces the simulated Fig 4g with the analytical curve, overlays the
simulation as validation, and writes the rate x duration lookup table the
...
Panels
------
a  Minimum passing firing rate vs recording duration, one trace per confidence
   threshold (the analytical Fig 4g), with simulated pass rates overlaid.
...
c  tau_pass0, the shortest violation-free window that would let a unit pass,
   against firing rate; the shaded band above 10 ms marks units that cannot
   pass whatever their autocorrelogram looks like.
...
def main():
    ...
    for g, c in zip(confs, cols):
        ax.plot(dur_h, min_passing_fr(dur_h * 3600, 0.003, conf_thresh=g),
                color=c, label=f"{g}")
    # simulation overlay: the lowest simulated rate that passes at 90%
    cal = SIMS / "calibration.pqt"
    if cal.exists():
        d = pd.read_parquet(cal)
        clean = d[(d.cont_prop == 0.08) & (d.rp_dur == 0.003)]
        for rd, grp in clean.groupby("rec_dur"):
            grp = grp.sort_values("total_rate")
            ok = grp[grp.sliding_pass_90 > 0.5]
            if len(ok):
                ax.plot(rd / 3600, ok.total_rate.iloc[0], "kv", ms=5,
                        zorder=5, label="Simulation" if rd == 1800 else None)
    ax.set_xlabel("Recording duration (h)")
    ax.set_ylabel("Minimum passing firing rate (spikes/s)")
    ...
    ax.text(0.13, 22, "Cannot pass: beyond the tested window",
            fontsize=7, va="bottom")
    ax.set_xscale("log"); ax.set_yscale("log")
    ax.set_ylim(0.01, 1000)
    ax.set_xlabel("Firing rate (spikes/s)")
    ax.set_ylabel("Shortest passing clean window, tau_pass0 (ms)")
    ...
    lines = ["Minimum firing rate (spikes/s) for a unit with no observed",
             "refractory period violations to pass, at a 10% contamination",
    ...
    lines += ["", "tau_pass0: the shortest violation-free window that would let a",
              "unit pass (10% contamination, 90% confidence, 1 h recording):", ""]
    for f in (0.2, 0.3, 0.5, 0.8, 1.0, 1.1, 1.5, 2.0, 5.0):
        t = tau_pass0(f * 3600, 3600) * 1000
        tag = "  <- cannot pass (window is 10 ms)" if t > 10 else ""
